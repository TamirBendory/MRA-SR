...
\end{eqnarray}
The largest non-zero frequency $\tB$ is called the bandlimit of the signal.
We aim to recover the signal $x$ from $y$. The rotations are referred to as \emph{nuisance variables}---while they are unknown,the goal of the task is not estimating them.
...
In this work, we are interested in  the \emph{high-noise, few samples regime}  in which $\sigma$ is large  and $L\leq 2\tB$. In particular, we study the interplay between four parameters: the bandlimit $\tB$, the number of samples $L$, the noise level $\sigma$, and the number of measurements $N$. %We denote the ratio $K=\tB/L$ as the \emph{super-resolution factor}.
...
\end{equation}
almost surely. 
Thus, to simplify notation, we take expectation over the invariants in the lemma. In addition, in expectation, the effect of the noise reduces to known bias terms that can be removed easily (see for instance~\cite{bendory2017bispectrum}). \TODO{to discuss the bias explicitly} Hence, for the sake of clarity, the lemma ignores them.
